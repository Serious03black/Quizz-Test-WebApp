...
const TestPage = () => {
  ...
  const studentInfo = JSON.parse(localStorage.getItem('studentInfo') || '{}'); // Load student info from localStorage
  ...
  const handleSubmit = () => {
    if (!studentInfo.name) {
      alert('Student name not found. Please complete the form first.');
      navigate('/'); // Redirect to MCQForm if name is missing
      return;
    }

    const submissionData = {
      name: studentInfo.name, // Include student name
      answers: answers, // Array of answers
    };
    console.log('Submitting data:', submissionData); // Log for debugging

    axios
      .post('http://localhost:5000/api/submit', submissionData)
      .then((response) => {
      ...
      .catch((error) => {
        ...
        if (error.response) {
          alert(`Submission failed: ${error.response.data.error || error.response.data}`);
        } else {
          alert('Network error: Could not submit test results.');
        }
      });
  };
